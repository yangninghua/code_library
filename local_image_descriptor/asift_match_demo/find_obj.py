...
def explore_match(win, img1, img2, kp_pairs, status = None, H = None):
    ...
    if H is not None:
        ...
        cv.polylines(vis, [corners], True, (255, 255, 255), thickness=8, lineType=8)
    ...
    def onmouse(event, x, y, flags, param):
        ...
        if flags & cv.EVENT_FLAG_LBUTTON:
            ...
            for i in idxs:
                ...
                try:
                    if status[i][0]:
                        cv.line(cur_vis, (x1, y1), (x2, y2), green)
                    else:
                        cv.line(cur_vis, (x1, y1), (x2, y2), red)
                except:
                    cv.line(cur_vis, (x1, y1), (x2, y2), red)
                kp1, kp2 = kp_pairs[i]
                kp1s.append(kp1)
...
def main():
    ...
    img1 = cv.imread(cv.samples.findFile(fn1), -1)
    img2 = cv.imread(cv.samples.findFile(fn2), -1)
