...
class Munkres(object):
    ...
    def _step_1(self):
        ...
        对于矩阵的每一行，找到最小的元素并从其行中的每个元素中减去它。转到步骤2。
        """
    ...
    def _step_3(self):
        ...
        覆盖包含0的每一列，如果覆盖的K==row，则描述了一组完整的唯一分配，在这种情况下，转到完成，否则转到步骤4。
        """
    ...
    def _find_uncovered_zero(self):
        ...
        返回矩阵中未覆盖零的 (行，列) 按顺序返回也可以随机返回， 如果没有未覆盖的零，则返回无
        """
        zeros = self.matrix.zeros()
        for (i, j) in zeros:
    ...
    def _step_4(self):
        ...
        找到一个未覆盖的零并将其素数。 如果没有带星号的零
        包含这个素数零的行，转到第 5 步。否则，覆盖
        这一行并揭开包含星号零的列。 以这种方式继续，直到没有未发现的零点为止。 保存最小的未覆盖值并转到步骤 6。
        """
        done = False
        while not done:
    ...
    def _step_5(self):
        ...
        构造一系列交替的带撇号和星号的零，如下所示。
        让 Z0 代表在步骤 4 中发现的未覆盖的素数零。让 Z1
        表示 Z0 列中带星号的零（如果有）。 让 Z2 表示
        Z1 行中的素数零（总会有一个）。 继续直到系列终止于没有星号零的素数零
        在其列中。 取消星标每个系列的零星，每个星标系列的素数零，擦除所有素数并揭开中的每一行矩阵。返回步骤 3。
        """
    ...
    def _step_6(self):
        ...
        将步骤 4 中找到的值添加到每个覆盖行的每个元素，并从每个未覆盖列的每个元素中减去它。 
        返回第 4 步，不更改任何星号、素数或覆盖线。
        """
        minval = INFINITY
...
def random_test_munkres(values, values_dict, nrows, ncols):
    """
    ...
    print("len(munkres_match): ", len(munkres_match), "\n")
    print("munkres_match: ", munkres_match, "\n")
    print("munkres_weight: ", munkres_weight, "\n")
...
if __name__ == '__main__':
    ...
    cost_dict = {}
    cost_list = []
    for k,v in enumerate(cost_mat):
        for p,q in enumerate(v):
            cost_dict[(k,p)] = q
            cost_list.append((k,p,q))

    nrows = len(cost_mat)
    ncols = len(cost_mat[0])
    random_test_munkres(cost_list, cost_dict, nrows, ncols)